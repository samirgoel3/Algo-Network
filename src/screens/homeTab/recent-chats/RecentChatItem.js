...
const RecentChatItem = ({itemData, navigation})=>{
    return(
        ...
        >
            <Image source={{uri:itemData.user_image}} style={{width:50, height:50, borderRadius:50, borderColor:'#ffffff', borderWidth:1}}/>
            <View style={{ flex:1, paddingHorizontal:10}}>
                <Text style={{fontFamily:FONT.BOLD, color:Colors.WHITE, fontSize:18}}>{itemData.name}</Text>
                <Text style={{fontFamily:FONT.SEMI_BOLD, color:Colors.GREY, fontSize:15}}>{itemData.last_message}</Text>
            </View>

            <View style={{ alignItems:'center', justifyContent:'center'}}>
                <Text style={{fontFamily:FONT.REGULAR, color:Colors.GREY}}>{itemData.time}</Text>
